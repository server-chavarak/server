...
// ฟังก์ชันกรองข้อมูลสำหรับแผนก Spiral
function filterData($filterType, $date) {
    global $conn;
    $departmentData = [];

    // SQL Query เฉพาะแผนก Spiral (S_ID = 1)
    switch ($filterType) {
        case 'day':
            $sql = "SELECT S_ID, SUM(currentPlan) AS currentPlan, SUM(FinishGood) AS FinishGood, DATE(Date_Time) AS Date_Time 
                    ...
                    WHERE DATE(Date_Time) = '$date' AND S_ID = 1 
        ...
        case 'month':
            $sql = "SELECT S_ID, SUM(currentPlan) AS currentPlan, SUM(FinishGood) AS FinishGood, DATE(Date_Time) AS Date_Time 
                    FROM actual_production 
                    WHERE YEAR(Date_Time) = YEAR('$date') AND MONTH(Date_Time) = MONTH('$date') AND S_ID = 1 
                    GROUP BY S_ID, DATE(Date_Time)
                    ORDER BY S_ID, Date_Time";
            break;
        case 'year':
            $sql = "SELECT S_ID, SUM(currentPlan) AS currentPlan, SUM(FinishGood) AS FinishGood, 
                    ...
                    FROM actual_production 
                    WHERE YEAR(Date_Time) = YEAR('$date') AND S_ID = 1 
                    GROUP BY S_ID, Month 
                    ORDER BY S_ID, Month";
            break;
        default:
            $sql = "SELECT S_ID, SUM(currentPlan) AS currentPlan, SUM(FinishGood) AS FinishGood, DATE(Date_Time) AS Date_Time 
                    FROM actual_production 
                    WHERE DATE(Date_Time) = CURDATE() AND S_ID = 1
                    GROUP BY S_ID, DATE(Date_Time) 
                    ORDER BY S_ID, Date_Time";
            break;
    }
...
<head>
    <meta charset="UTF-8">
    <title>Spiral Production Summary</title>
...
<body>
<h1>Spiral Production Summary</h1>
...
<div class="chart-container">
    <div class="chart-box">
        <h2>Spiral</h2>
        <canvas id="chart-spiral" width="900" height="300"></canvas>
    </div>
    <script>
        const ctxSpiral = document.getElementById('chart-spiral').getContext('2d');
