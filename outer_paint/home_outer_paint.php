...
<head>
    ...
    .filter-container inouter_paintt {
        padding: 10px;
...
<div class="chart-container">
    ...
    function calculateColorAndDisplay($finishGood, $currentPlan, $totalHours, $greenPercentage, $orangePercentage, $redPercentage, $workStartTime, $workEndTime, $productQueue, $st_id) {
        ...
        $expectedOutouter_paintt = ($hoursPassed / $totalHours) * $currentPlan; // ชิ้นที่ควรผลิตได้ในเวลาที่ผ่านมา

        // คำนวณเปอร์เซ็นต์การผลิตที่เสร็จสมบูรณ์
        if ($expectedOutouter_paintt > 0) {
            $percentComplete = ($finishGood / $expectedOutouter_paintt) * 100;
