...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!empty($product_id) && !empty($date_time) && !empty($plans)) {
        ...
        header('Location: add_fitting_actual.php');  // Redirect
        exit();
    } else {
        $_SESSION['alertMessage'] = "กรุณากรอกข้อมูลให้ครบถ้วน";
        $_SESSION['alertType'] = 'error';
        header('Location: add_fitting_actual.php');
        exit();
    }
}
...
$product_query = "SELECT DISTINCT p.Product_ID, 
    ...
    WHERE (SELECT s.S_Name FROM work_step ws
           ...
           ORDER BY ws.St_ID LIMIT 1) = 'Fitting'";  // เงื่อนไขให้แสดงเฉพาะผลิตภัณฑ์ที่เริ่มที่แผนก Fitting
...
<body>
    <a href="../admin/spiral.php" class="back-link">ย้อนกลับ</a>
    <form action="add_fitting_actual.php" method="POST" class="addcustomer">
        ...
        if (isset($_SESSION['alertMessage'])) {
            ...
            echo "Swal.fire({
                ...
                if (result.isConfirmed) {
                    window.location.href = 'fitting.php';  // เปลี่ยนเส้นทางไปยัง spiral.php หลังจากกดตกลง
                }
            });";
