...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!empty($pipeTypeName)) {
        ...
        $checkQuery = "SELECT * FROM add_tname WHERE Name_type = ?";
        ...
        if ($resultCheck->num_rows > 0) {
            ...
            $stmt = $conn->prepare("INSERT INTO add_tname (Name_type) VALUES (?)");
...
$typeList = [];
$query = "SELECT ID, Name_type FROM add_tname";
...
<body>
        ...
        <div class="table-container">
            <table>
                ...
                <tbody id="typeTableBody">
                    ...
                    <tr id="type-<?php echo htmlspecialchars($type['ID']); ?>">
                        <td><?php echo $type['ID']; ?></td>
                        <td><?php echo htmlspecialchars($type['Name_type']); ?></td>
                        <td class="action">
                            <a href="edit_tname.php?ID=<?php echo htmlspecialchars($type['ID']); ?>" class="edit-button">
                                <i class="fa fa-pen"></i>
                            </a>
                            <a href="#" onclick="confirmDeletetype('<?php echo htmlspecialchars($type['ID']); ?>')" class="delete-button">
    ...
    <script>
        ...
        document.getElementById('confirmDelete').onclick = function () {
            if (typeToDelete) {
                fetch('delete_tname.php', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/x-www-form-urlencoded'
                    },
                    body: new URLSearchParams({ ID: typeToDelete })
                })
