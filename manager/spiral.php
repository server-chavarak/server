...
$S_ID = 1; // ค่า S_ID สำหรับแผนก Spiral (สามารถปรับค่า S_ID สำหรับแผนกอื่น ๆ)
                    ...
                    <tr id="actual_production-<?php echo htmlspecialchars($actual_production['Act_ID']); ?>" class="<?php echo $is_disabled; ?>">
                        ...
                        <td class="action">
                            <?php if ($actual_production['Product_queue'] > 0 || $actual_production['FinishGood'] > 0): ?>
                                <a href="edit_spiral.php?Act_ID=<?php echo htmlspecialchars($actual_production['Act_ID']); ?>" class="edit-button">
...
<div id="deleteModal" class="modal">
    <div class="modal-delete">
        <p>คุณแน่ใจหรือไม่ว่าต้องการลบการผลิตนี้?</p>
...
<script>
    // Function to confirm deletion of an actual_production
let Act_IDToDelete = null;

function confirmDelete(Act_ID) {
    Act_IDToDelete = Act_ID;
    const modal = document.getElementById('deleteModal');
    if (modal) {
        modal.style.display = 'flex'; // เปลี่ยน display ของ modal เป็น flex เมื่อเรียกใช้งาน
    }
}
...
document.getElementById('cancelDelete').onclick = function() {
    const modal = document.getElementById('deleteModal');
    if (modal) {
        modal.style.display = 'none'; // เปลี่ยน display ของ modal กลับเป็น none เมื่อปิด
    }
    Act_IDToDelete = null;
}
...
document.getElementById('confirmDelete').onclick = function() {
    if (Act_IDToDelete) {
        fetch('delete_actual.php', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
            body: JSON.stringify({ Act_ID: Act_IDToDelete })
        })
        .then(response => {
            if (response.ok) {
                document.getElementById('actual_production-' + Act_IDToDelete).remove();
            } else {
                response.text().then(text => {
                    console.error('Error deleting actual_production:', text);
                });
            }
            document.getElementById('deleteModal').style.display = 'none';
            Act_IDToDelete = null;
        })
        .catch(error => console.error('Error deleting actual_production:', error));
    }
}
