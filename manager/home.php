...
$sql = "SELECT S.S_ID, S.S_Name, ws.St_Name, ws.St_ID, 
                ...
                AP.Date_Time, 
                AP.product_queue -- เพิ่ม product_queue
        ...
        GROUP BY S.S_ID, ws.St_Name, AP.Date_Time, AP.product_queue -- เพิ่ม product_queue ใน group by
        ...
        <select id="filter-product" onchange="applyFilters()" <?php echo empty($filterWO) ? 'disabled' : ''; ?>>
            ...
            if (!empty($filterWO)) {
                $product_query = "SELECT 
                            ...
                            LEFT JOIN product_detail pd ON pr.P_ID = pd.P_ID    -- เชื่อมกับ product_detail เพื่อดึง P_Name
                            LEFT JOIN type t ON pr.T_ID = t.T_ID                -- เชื่อมกับ type เพื่อดึงข้อมูลเพิ่มเติม
                            LEFT JOIN type_detail td ON t.TD_ID = td.TD_ID      -- เชื่อมกับ type_detail เพื่อดึง TD_Name
                            LEFT JOIN pipeend_detail pe ON t.PE_ID = pe.PE_ID   -- เชื่อมกับ pipeend_detail เพื่อดึง PE_Name
                            WHERE od.WO_No = ?";
...
<div class="chart-container">
   ...
   while ($row = $result->fetch_assoc()) {
        ...
        $productQueue = $row['product_queue'] ?? 0; // ดึงค่า product_queue จากฐานข้อมูล
        $st_id = $row['St_ID']; // เก็บค่า St_ID
