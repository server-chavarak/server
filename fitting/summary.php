...
// ฟังก์ชันกรองข้อมูลสำหรับแผนก fitting
function filterData($filterType, $date) {
    global $conn;
    $departmentData = [];

    // SQL Query เฉพาะแผนก fitting (S_ID = 2)
    switch ($filterType) {
        case 'day':
            $sql = "SELECT S_ID, SUM(currentPlan) AS currentPlan, SUM(FinishGood) AS FinishGood, DATE(Date_Time) AS Date_Time 
                    ...
                    WHERE DATE(Date_Time) = '$date' AND S_ID = 2 
        ...
        case 'month':
            $sql = "SELECT S_ID, SUM(currentPlan) AS currentPlan, SUM(FinishGood) AS FinishGood, DATE(Date_Time) AS Date_Time 
                    FROM actual_production 
                    WHERE YEAR(Date_Time) = YEAR('$date') AND MONTH(Date_Time) = MONTH('$date') AND S_ID = 2 
                    GROUP BY S_ID, DATE(Date_Time)
                    ORDER BY S_ID, Date_Time";
            break;
        case 'year':
            $sql = "SELECT S_ID, SUM(currentPlan) AS currentPlan, SUM(FinishGood) AS FinishGood, 
                    ...
                    FROM actual_production 
                    WHERE YEAR(Date_Time) = YEAR('$date') AND S_ID = 2 
                    GROUP BY S_ID, Month 
                    ORDER BY S_ID, Month";
            break;
        default:
            $sql = "SELECT S_ID, SUM(currentPlan) AS currentPlan, SUM(FinishGood) AS FinishGood, DATE(Date_Time) AS Date_Time 
                    FROM actual_production 
                    WHERE DATE(Date_Time) = CURDATE() AND S_ID = 2
                    GROUP BY S_ID, DATE(Date_Time) 
                    ORDER BY S_ID, Date_Time";
            break;
    }
...
<head>
    <meta charset="UTF-8">
    <title>fitting Production Summary</title>
...
<body>
<h1>Fitting Production Summary</h1>
...
<div class="chart-container">
    <div class="chart-box">
        <h2>Fitting</h2>
        <canvas id="chart-fitting" width="900" height="300"></canvas>
    </div>
    <script>
        const ctxfitting = document.getElementById('chart-fitting').getContext('2d');

        <?php if ($filterType === 'day') { ?>
        // กรณีรายวัน แสดงแท่งคู่สำหรับแผนผลิตและผลิตจริง พร้อมวันที่ใต้กราฟ
        new Chart(ctxfitting, {
            type: 'bar',
            data: {
