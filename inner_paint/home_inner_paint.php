...
<head>
    ...
    .filter-container ininner_paintt {
        padding: 10px;
...
<div class="chart-container">
    ...
    function calculateColorAndDisplay($finishGood, $currentPlan, $totalHours, $greenPercentage, $orangePercentage, $redPercentage, $workStartTime, $workEndTime, $productQueue, $st_id) {
        ...
        $expectedOutinner_paintt = ($hoursPassed / $totalHours) * $currentPlan; // ชิ้นที่ควรผลิตได้ในเวลาที่ผ่านมา

        // คำนวณเปอร์เซ็นต์การผลิตที่เสร็จสมบูรณ์
        if ($expectedOutinner_paintt > 0) {
            $percentComplete = ($finishGood / $expectedOutinner_paintt) * 100;
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        $productQueue = isset($productQueue) ? $productQueue : 0; 
        $st_id = isset($st_id) ? $st_id : null;

        // คำนวณสีตามจำนวนชิ้นที่ผลิตได้เทียบกับแผน
        $color = calculateColorAndDisplay($finishGood, $currentPlan, $totalHours, $greenPercentage, $orangePercentage, $redPercentage, $workStartTime, $workEndTime, $productQueue, $st_id);
    ?>
